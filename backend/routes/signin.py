...
from utils.auth_utils import create_tokens, blacklist_token, generate_fingerprint, JWT_SECRET, JWT_ALGORITHM
import jwt
import hashlib
    ...
    try:
        with DatabaseConnection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                # 5. Issue JWT and Refresh tokens
                ...
                resp.set_cookie('access_token', access_token, httponly=True, samesite='Strict', max_age=15*60)
                resp.set_cookie('refresh_token', refresh_token, httponly=True, samesite='Strict', max_age=7*24*60*60)
    ...
    try:
        ...
        with DatabaseConnection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                # 5. Issue new pairs
                new_access, new_refresh = create_tokens(user)
                
                resp = make_response(jsonify({"message": "Session renewed"}))
                resp.set_cookie('access_token', new_access, httponly=True, samesite='Strict', max_age=15*60)
                resp.set_cookie('refresh_token', new_refresh, httponly=True, samesite='Strict', max_age=7*24*60*60)
                return resp, 200
