    ...
    try:
        with DatabaseConnection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                # 1. Deduplicate: Check if active/verified user already exists on this role
                cur.execute("SELECT id FROM users WHERE email = %s AND role = %s", (email, role))
                if cur.fetchone():
                    return jsonify({"error": f"An account with this email already exists for the role: {role}"}), 409
